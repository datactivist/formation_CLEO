...
<p>
Pad collaboratif : https://frama.link/cleo3
</p>
...
- appréhender les questions de recherche
- acquérir des données "en conditions réelles" (solr, logs)
- visualiser et analyser des distributions
- cartographier
- éléments de text mining
...
## Interroger solr depuis R

- http://search.openedition.org/ => recherche tous documents OE
- motorisé par solr
- on peut interroger solr depuis R, grâce à [`solrium`](https://github.com/ropensci/solrium)
...
solr_search(q='*:*', rows = 20)
```

## Interroger solr depuis R

```{r}
solr_search(q='texte:gombin', rows = 100, sort = "datepubli asc", fl = c("naked_titre", "contributeur_auteur", "site_name", "url", "datepubli", "site_type")) %>% glimpse
```

## Interroger solr depuis R

```{r}
solr_search(q='naked_texte:"gombin"', rows = 500, sort = "datepubli asc", fl = c("naked_titre", "contributeur_auteur", "naked_texte", "site_name", "url", "datepubli", "site_type")) %>% glimpse
```

## Interroger solr depuis R

```{r}
solr_facet(q = 'texte:gombin', facet.field = "site_name")$facet_fields$site_name
```

# logs

## Lire des logs bruts dans R

```{r, message=FALSE, warning=FALSE}
logs <- read_log("../data/12012016-TOTAL-REVUES.log")
names(logs) <- c("IP", "V1", "V2", "date_time", "request", "status", "bytes", "referrer", "user_agent")
logs <- logs %>% select(-V1, -V2)
logs %>% glimpse
```
...
```{r}
library(stringr)
logs <- data_frame(path = list.files("../data/")[str_detect(list.files("../data/"), "12012016")]) %>% 
  rowwise %>% 
  do({
    tmpname <- .$path
    tmp <- read_log(paste0("../data/", tmpname))
    names(tmp) <- c("IP", "V1", "V2", "date_time", "request", "status", "bytes", "referrer", "user_agent")
    tmp <- tmp %>% 
      select(-V1, -V2) %>% 
      filter(status %in% 200) %>% 
      select(-status)
    tmp$file <- tmpname
...
logs %>% 
  sample_n(100000) %>% 
  mutate(domaine = re_matches(request, re)$domaine) %>% 
  filter(!is.na(domaine)) %>% 
  filter(!str_detect(domaine, "openedition.org")) %>% 
  group_by(file, domaine) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n)) %>% 
  mutate(domaine = factor(domaine, levels = unique(domaine))) %>% 
  ggplot(aes(x = n, group = file)) +
  stat_ecdf(aes(color = file)) +
  theme_bw() +
...
deciles <- logs %>% 
  ...
  arrange(desc(n)) %>% 
  mutate(ecdf = cumsum(n) / sum(n)) %>% 
  do(data_frame(probs = seq(0, 1, by = 0.1), 
                quantiles = quantile(.$n, probs = seq(0, 1, by = 0.1))
                ))
```
# Cartographie
...
- des capacités de base grâce à [`sp`](https://cran.r-project.org/web/packages/sp/vignettes/intro_sp.pdf) (et désormais [`sf`](https://github.com/edzer/sfr))
